...
//		));
    ...
    public function status_journey() {

        return $this->db->select('SELECT * FROM journey_for_bus WHERE status = "Ada" ');
    }

    public function searchAvailablelBus($from, $to, $date) {
        $sts="Ada";
        $tgl_now = $date;
        $availablelBus = $this->db->select('SELECT
                ...
                journey.journeyNo,
                bus.busModel,
                sopir_dtl.foto_mobil
               
                FROM journey
                JOIN journey_for_bus ON journey.journeyNo = journey_for_bus.journeyNo
                JOIN bus ON journey_for_bus.busNo = bus.busNo
                JOIN sopir_dtl ON bus.busNo=sopir_dtl.no_mobil
                WHERE journey.journeyFrom ="' . $from . '" AND journey.journeyTo ="' . $to . '" AND journey_for_bus.status ="'.$sts.'"AND journey_for_bus.tgl_posisi ="'.$tgl_now.'" ');
        ...
        if (isset($availablelBus)) {
            foreach ($availablelBus as $key => $value) {
                ...
                $arrayData[$i]['busModel'] = $value['busModel'];
                $arrayData[$i]['foto_mobil'] = $value['foto_mobil'];
                ...
                $rating_mobil = $this->db->select('SELECT 
                        AVG(rating_mobil.rate)as jml
                        FROM rating_mobil
                        JOIN bus ON rating_mobil.no_mobil = bus.busNo
                        JOIN journey_for_bus ON bus.busNo = journey_for_bus.busNo
                        WHERE journey_for_bus.busNo = "' . $value['busNo'] . '" ');
                $r = 0;
                if (isset($rating_mobil)) {
                    foreach ($rating_mobil as $key => $value3) {
                        $arrayDataEntry1[$r]['jml'] = $value3['jml'];
                        $r++;
                    }
                }
                $arrayData[$i]['rating_mobil'] = $arrayDataEntry1;
