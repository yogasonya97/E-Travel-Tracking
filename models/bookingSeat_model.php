...
//        $this->booker = new Booker();
    ...
    public function seatBookingConform($new_bookingID) {
     ...
     public function seatBookingConform_ticket_p($new_bookingID) {
        if (Session::get('bookingTime') == true) {
                $this->db->beginTransaction();
                try {
                    $i = 0;
                    ...
                    for ($x = 0; isset($array[$x]); $x++) {
                        $sth1 = $this->db->prepare('INSERT INTO receive_ticke_p (`ticketNo`,`passengerName`,`seatNo`,`gender`,`bookingID`)
                     VALUES 
                     ("' . $array[$x]['ticketNo'] . '","' . $array[$x]['passengerName'] . '","' . $array[$x]['seatNo'] . '","' . $array[$x]['gender'] . '","' . $array[$x]['bookingID'] . '")');
                        $val1 = $sth1->execute();
    ...
    public function seatBookingConform_transfer_acc($new_bookingID,$ticketNo,$passengerName,$seatNo,$gender,$new_nobooking) {
        if (Session::get('bookingTime') == true) {
                $this->db->beginTransaction();
                try {
                    $i = 0;
                    ...
                    Session::deset('sessionforSelectin_s_tot_price');
                        ...
                        $sth1 = $this->db->prepare('INSERT INTO receive_ticke (`ticketNo`,`passengerName`,`seatNo`,`gender`,`bookingID`)
                     VALUES 
                    ("' . $ticketNo . '","' . $passengerName . '","' . $seatNo . '","' . $gender . '","' . $new_nobooking . '")');
                        $val1 = $sth1->execute();
    ...
    public function seatBookingConform_transfer_acc1($new_bookingID,$ticketNo,$passengerName,$seatNo,$gender,$new_nobooking) {
        if (Session::get('bookingTime') == true) {
                $this->db->beginTransaction();
                try {
                    $i = 0;
    ...
    public function seatBookingConform_transfer($new_bookingID,$rek_id) {
        if (Session::get('bookingTime') == true) {
                $this->db->beginTransaction();
                try {
                    $i = 0;
                    ...
                    $sth3 = $this->db->prepare('UPDATE booking SET metode_bayar = "Transfer Bank", payment_status = "P", rek_id_bank = "'.$rek_id.'" WHERE bookingID ="' . $array[0]['bookingID'] . '" ');
                    $val3 = $sth3->execute();
                    $this->db->commit();
                    return $array[0]['bookingID'];
                } catch (Exception $e) {
     ...
     public function seatBookingConform_cod($new_bookingID) {
        if (Session::get('bookingTime') == true) {
                $this->db->beginTransaction();
                try {
                    $i = 0;
                    ...
                    $sth3 = $this->db->prepare('UPDATE booking SET metode_bayar = "COD", payment_status = "P", rek_id_bank = "005" WHERE bookingID ="' . $array[0]['bookingID'] . '" ');
                    $val3 = $sth3->execute();
                    $this->db->commit();
                    return $array[0]['bookingID'];
                } catch (Exception $e) {
