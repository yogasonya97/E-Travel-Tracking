...
class Journey_Model extends Model {
    ...
    public function notiforder() {
       ...
       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT COUNT(bookingID) AS jum_order FROM booking WHERE payment_status='P'");
    }
    public function listorder() {

       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT *, DATE_FORMAT(A.s_bookin_time,'%d %M %Y') AS tgl,B.bookerName,A.payment_status FROM booking A INNER JOIN booker B ON A.bookerNICNo=B.bookerNICNo WHERE A.payment_status='P' order by date(A.s_bookin_time) desc");
    }
...
//        exit();
      ...
      public function all_car() {
        return $this->db->select('SELECT COUNT(busNo) as car FROM bus');
    }
    public function all_sopir() {
        return $this->db->select('SELECT COUNT(id_sopir) as sopir FROM system_user');
    }
    public function all_informasi() {
        return $this->db->select('SELECT * FROM informasi');
    }
    public function jml_pelanggan() {
        return $this->db->select('SELECT COUNT(bookerNICNo) as plg FROM booker');
    }
