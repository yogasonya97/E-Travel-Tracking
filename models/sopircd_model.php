...
class Sopircd_Model extends Model {
...
//                                  ' );
  ...
  //   }
   ...
   public function count_jurusan($username) {
       ...
       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT COUNT(D.busNo) AS hitung FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo where privilege='SopirCD' AND userName= '$username' ");
    }

     public function count_jurusan_plg($username) {

       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT COUNT(D.busNo) AS hitung FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo INNER JOIN journey E ON D.journeyNo=E.journeyNo where A.privilege='SopirCD' AND A.userName= '$username' AND E.journeyFrom='Palembang'");
    }

      public function count_jurusan_lpg($username) {

       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT COUNT(D.busNo) AS hitung FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo INNER JOIN journey E ON D.journeyNo=E.journeyNo where A.privilege='SopirCD' AND A.userName= '$username' AND E.journeyFrom='Lampung'");
    }
    ...
    public function listorder() {
     ...
     public function get_status_istirahat($username) {
       ...
       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT D.status FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo where A.privilege='SopirCD' AND D.status='Istirahat' AND A.userName= '$username' ");
    }
    ...
    public function di_lampung($username) {
     ...
     public function get_akun_sopir($username) {
       ...
       //$j_No=$this->setJourneyNo($data['cRouteNo'],$data['cJourneyFrom'],$data['cJourneyTo']);
       return $this->db->select("SELECT * FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN sopir_cadangan D ON B.id_sopir_cadangan=D.id_sopir_cadangan where A.privilege='SopirCD' AND A.userName='$username' ");
    }

    public function get_jurusan_mobil($username) {
        return $this->db->select("SELECT * FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo where privilege='SopirCD' AND userName= '$username' ");
    }

    public function get_jurusanstatus_ada($username) {
        return $this->db->select("SELECT * FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo INNER JOIN journey E ON D.journeyNo=E.journeyNo where privilege='SopirCD' AND userName= '$username' AND D.status='Ada' ");
    }

    public function get_jurusanstatus_Tidakada($username) {
        return $this->db->select("SELECT * FROM system_user A INNER JOIN sopir_dtl B ON A.id_sopir=B.id_sopir INNER JOIN bus C ON B.no_mobil=C.busNo INNER JOIN journey_for_bus D ON C.busNo=D.busNo where privilege='SopirCD' AND userName= '$username' AND D.status='Tidak Ada' ");
    }
